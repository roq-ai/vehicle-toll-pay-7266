...
import { useOperatorFindFirst } from 'lib/roq';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['system-administrator', 'toll-operator'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useOperatorFindFirst(
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Operators',
      path: isTenantUser ? `/operators/view/${data?.id}` : '/operators',
      entity: 'operator',
      service: AccessServiceEnum.PROJECT,
      icon: FiHeadphones,
    },
    { name: 'Journeys', path: '/journeys', entity: 'journey', service: AccessServiceEnum.PROJECT, icon: FiCompass },
