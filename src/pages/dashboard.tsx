...
import { useRoqPlatformUserProfiles, usePaymentCount, useTollBoothCount, useJourneyCount } from 'lib/roq';
...
function DashboardPage() {
  ...
  const tollBoothStat = useTollBoothCount();

  const journeyStat = useJourneyCount();
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Toll Booths',
      result: tollBoothStat?.data,
      isLoading: tollBoothStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Journeys',
      result: journeyStat?.data,
      isLoading: journeyStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
