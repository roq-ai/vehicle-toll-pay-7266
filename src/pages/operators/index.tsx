...
import { useRoqClient, useOperatorFindManyWithCount } from 'lib/roq';
...
export function OperatorListPage(props: OperatorListPageProps) {
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['user', 'toll_booth.count'],
          searchTerm: params.searchTerm,
          searchTermKeys: [
            'description.contains',
            'operational_hours.contains',
            'location.contains',
            'status.contains',
            'name.contains',
          ],
  ...
  const { data, error, isLoading, mutate } = useOperatorFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.operator.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('toll_booth', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => record?._count?.toll_booth,
        }
      : null,
